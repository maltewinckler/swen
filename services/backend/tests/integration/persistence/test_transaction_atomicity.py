...
def atomicity_engine() -> AsyncEngine:
    """Create an in-memory SQLite engine for atomicity tests."""
...
@pytest.fixture(scope="module")
def atomicity_session_maker(atomicity_engine: AsyncEngine) -> async_sessionmaker:
...
@pytest.fixture(scope="module")
async def atomicity_tables(atomicity_engine: AsyncEngine):
    """Create database tables for atomicity tests."""
    async with atomicity_engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)
    yield
    async with atomicity_engine.begin() as conn:
        await conn.run_sync(Base.metadata.drop_all)
    await atomicity_engine.dispose()
