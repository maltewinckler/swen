...
@pytest.fixture
def credential_repository(integration_session, encryption_service):
    """Create credential repository for default test user."""
    return make_credential_repository(integration_session, encryption_service, TEST_USER_CONTEXT)
...
class TestCredentialPersistenceBasic:
    ...
    async def test_saved_credentials_are_encrypted_in_database(
        ...
        await integration_session.flush()  # Flush to make data visible for raw SQL
...
class TestMultipleCredentials:
    ...
    async def test_credentials_isolated_between_users(
        ...
        user1_repo = make_credential_repository(integration_session, encryption_service, TEST_USER_CONTEXT)
        user2_repo = make_credential_repository(integration_session, encryption_service, TEST_USER_CONTEXT_2)
