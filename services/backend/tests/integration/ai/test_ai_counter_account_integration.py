...
class TestTransactionImportWithAIIntegration:
    ...
    @pytest_asyncio.fixture
    async def import_service_with_ai(self, repositories, bank_account_service, setup_bank_account):
    ...
    async def test_import_multiple_transactions_classifies_correctly(
        ...
        assert len(account_ids) >= 2, "Expected transactions to be classified differently"
