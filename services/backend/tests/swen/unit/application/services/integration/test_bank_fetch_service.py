...
class TestTanConfiguration:
    ...
    @pytest.mark.asyncio
    async def test_tan_callback_wrapped_and_set(self):
        adapter = _make_adapter()
        service = BankFetchService(bank_adapter=adapter)

        sync_callback = MagicMock(return_value="123456")
        ...
        # set_tan_callback must have been called with an awaitable wrapper
        adapter.set_tan_callback.assert_awaited_once()
        wrapped = adapter.set_tan_callback.await_args[0][0]
        assert callable(wrapped)

    @pytest.mark.asyncio
    async def test_tan_callback_not_set_when_none(self):
        adapter = _make_adapter()
        service = BankFetchService(bank_adapter=adapter)
        ...
        adapter.set_tan_callback.assert_not_called()

    @pytest.mark.asyncio
    async def test_async_tan_callback_wrapped_correctly(self):
        """An already-async callback is also accepted and wrapped."""
        adapter = _make_adapter()
        service = BankFetchService(bank_adapter=adapter)

        async def async_callback(challenge):
            return "654321"
    ...
    @pytest.mark.asyncio
    async def test_tan_callback_for_fetch_accounts(self):
        adapter = _make_adapter()
        service = BankFetchService(bank_adapter=adapter)

        sync_callback = MagicMock(return_value="999")

        await service.fetch_accounts(_make_credentials(), tan_callback=sync_callback)
