...
async def test_net_income_over_time_calculates_income_minus_expenses(
    ...
    salary = _mk_account(
        user_id=current_user.user_id, name="Salary", account_type="income"
    )
    groceries = _mk_account(
        user_id=current_user.user_id,
        name="Groceries",
...
async def test_savings_rate_over_time_calculates_rate_and_average(
    ...
    groceries = _mk_account(
        user_id=current_user.user_id, name="Groceries", account_type="expense"
    )
    async_session.add_all([salary, groceries])
...
async def test_balance_history_over_time_tracks_asset_balances_and_sorting(
    ...
    async_session.add_all(
        [
            _mk_entry(
                tx_id=tx_checking.id, account_id=checking.id, debit=Decimal("5000.00")
            ),
            _mk_entry(
                tx_id=tx_savings.id, account_id=savings.id, debit=Decimal("10000.00")
            ),
            _mk_entry(
                tx_id=tx_expense.id, account_id=groceries.id, debit=Decimal("123.00")
            ),
        ],
    )
    await async_session.flush()
...
async def test_net_worth_over_time_assets_minus_liabilities(
    ...
    async_session.add_all(
        [
            _mk_entry(
                tx_id=tx_assets.id, account_id=checking.id, debit=Decimal("10000.00")
            ),
            _mk_entry(
                tx_id=tx_assets.id, account_id=savings.id, debit=Decimal("10000.00")
            ),
            _mk_entry_credit(
                tx_id=tx_liab.id, account_id=cc.id, credit=Decimal("2000.00")
            ),
        ],
    )
    await async_session.flush()
