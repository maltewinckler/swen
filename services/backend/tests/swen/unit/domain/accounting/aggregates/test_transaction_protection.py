...
class TestIsBankImport:
    ...
    def test_no_source_returns_false(
        self, user_id, checking_account, groceries_account, amount
    ):
...
class TestProtectedEntries:
    ...
    def test_bank_import_protects_asset_entries(
        self, bank_import_transaction, checking_account
    ):
...
class TestIsEntryProtected:
    ...
    def test_asset_entry_protected_in_bank_import(self, bank_import_transaction):
        ...
        asset_entry = next(
            e
            for e in bank_import_transaction.entries
...
class TestRemoveEntry:
    ...
    def test_remove_any_entry_from_manual_succeeds(self, manual_transaction):
        ...
        asset_entry = next(
            e
            for e in manual_transaction.entries
            if e.account.account_type == AccountType.ASSET
        )
...
class TestClearEntries:
    ...
    def test_clear_preserves_protected_entries_in_bank_import(
        self, bank_import_transaction
    ):
        ...
        assert (
            bank_import_transaction.entries[0].account.account_type == AccountType.ASSET
        )
...
class TestBankImportIncomeTransaction:
    ...
    @pytest.fixture
    def bank_income_transaction(
        self, user_id, checking_account, salary_account, amount
    ):
    ...
    def test_income_entry_not_protected(self, bank_income_transaction):
        ...
        income_entry = next(
            e
            for e in bank_income_transaction.entries
