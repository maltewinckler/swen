...
def mock_email_service():
    """Mock email adapter to capture sent emails.

    SmtpEmailNotificationAdapter is constructed once per request inside
    AdapterFactoryDefault.email_notification_port() (infrastructure layer),
    shared by both ForgotPasswordCommand and ResetPasswordCommand, so a
    single patch target covers both.
    """
    with patch(
        "swen_identity.infrastructure.adapters.adapter_factory.SmtpEmailNotificationAdapter"
    ) as mock_email_class:
