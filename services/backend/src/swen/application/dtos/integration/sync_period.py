"""Frozen value object describing a sync date range.

`SyncPeriod` carries the `(start_date, end_date)` window used by the sync
stack together with an `adaptive` flag. When `adaptive` is `True`, callers
expand the placeholder window per-IBAN via `SyncPeriodResolver`, and may
widen the resulting window to cover the actual range observed on the bank
side via :meth:`SyncPeriod.widen`.
"""
